...
RSpec.describe Quiz, :type => :model do
  ...
  before (:each) do
    ...
    @quiz = Quiz.create!(
      tags: @tags, question_count: 3, custom: false, name: '', user: @user)
  end
  ...
  it '#add_name_if_blank after_save callback works' do
    expect(@quiz.name).to eq '3-question quiz on one, two, three'
    @quiz = Quiz.create!(tags: [@tag1, @tag3], question_count: 10, custom: false)
    expect(@quiz.name).to eq '10-question quiz on one, three'
  end

  it 'Quiz#userless scope works' do
    q1 = Quiz.create!(tags: @tags, question_count: 1, custom: false, name: '', user: @user)
  ...
  context '#tag names' do
    it 'returns correctly with several tags' do
      expect(@quiz.tag_names).to eq 'one, two, three'
    end
  ...
  it '#regenerate_questions works' do
